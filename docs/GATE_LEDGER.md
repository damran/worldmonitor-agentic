...
| **G6** | Sensitive-topic guard is a hardcoded **denylist** (fails open for unmodelled topics) | 0020, **0047** | **slice-1 LANDED** (Gate E) | Denylist → **deny-by-default**: `guard/sensitivity.py` flags on `registry.topic.RISKS` (28 codes, programmatic) + off-ontology⇒sensitive; legacy literal deleted; reason-scoped approved-group exemption fence. Slice-2 (k-hop graph sensitivity + Chow abstain band) still pending. |
