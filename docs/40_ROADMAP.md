...
- [ ] **MEDIUM/LOW sweep** — #105 (edge-prov skip+dead-letter), M-5 (online-migration safety), ~~M-6 (landing GC)~~ (**CLOSED** ADR 0083 — reference-based orphan GC, report-only default + deletion opt-in, disk-growth gauges, deterministic-key invariant), ~~wikidata enricher via `guarded_stream`~~ (**CLOSED** ADR 0081), dig/nmap richer FtM map, ~~suffix-match allowlist~~ (**CLOSED** ADR 0082).
