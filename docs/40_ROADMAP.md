...
- [x] **H-8 remaining halves** (sliced; decided, ADR 0054) — [x] auto-hard-disable after N failures (ADR 0074, PR #132) · [x] periodic in-loop maintenance cadence (ADR 0075) · [x] resolve wall-clock timeout + lock-skip escalation (ADR 0075) · [x] Prometheus `/metrics` transport (ADR 0076).
- [ ] **Container/egress sandbox** — flips `container_sandbox_enabled`; unlocks nmap execution (ADR 0072 follow-up). **ADR 0077 (sandbox-runner sidecar)**: Slice 1 (app seam — settings + app-side `ContainerRunner` + `operator_run` refuse-or-route + the sidecar service code, behind the default-off flag) landed; Slice 2 (Dockerfile tool binaries + compose service + `CAP_NET_ADMIN`/nftables egress + resource limits) pending.
