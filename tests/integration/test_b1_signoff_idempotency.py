...
def _seed_parked(sessions: sessionmaker[Session], clean_graph: Neo4jClient, tenant_id: str) -> str:
...
def _counts(sessions: sessionmaker[Session], tenant_id: str) -> tuple[int, int, int]:
    """(pending_review queue rows, resolver_judgement rows, sign_off rows) for the tenant."""
    with sessions() as session:
        ...
        judgements = session.execute(
            select(func.count())
            .select_from(ResolverJudgement)
            .where(ResolverJudgement.tenant_id == tenant_id)
        ).scalar_one()
        signoffs = session.execute(
            select(func.count()).select_from(SignOff).where(SignOff.tenant_id == tenant_id)
        ).scalar_one()
...
def test_approve_crash_window_is_surfaced_and_recovers_idempotently(
    ...
    tenant_id = "b1-signoff-approve"
    engine = make_engine(postgres_dsn)
    create_all(engine)
    sessions = session_factory(engine)
    ensure_constraints(clean_graph)
    canonical_id = _seed_parked(sessions, clean_graph, tenant_id)
    ...
    with sessions() as session:
        parked = signoff.list_parked(session, tenant_id, clean_graph)
    ...
    assert _counts(sessions, tenant_id) == (2, 0, 0)

    # --- RECOVER: re-run the SAME approve. It must converge, not duplicate. ---
    with sessions() as session:
        result = signoff.approve(
            session, clean_graph, tenant_id=tenant_id, canonical_id=canonical_id, approver="alice"
        )
    ...
    parked_rows, judgements, signoffs = _counts(sessions, tenant_id)
    ...
    owns = clean_graph.execute_read(
        "MATCH (p:Person {tenant_id: $t})-[r:OWNS]->(:Company {tenant_id: $t}) "
    ...
    with sessions() as session:
        again = signoff.approve(
            session, clean_graph, tenant_id=tenant_id, canonical_id=canonical_id, approver="alice"
        )
    assert again.already_applied is True
    assert _counts(sessions, tenant_id) == (0, 1, 1), "no new judgement or sign-off row on re-run"
    engine.dispose()
...
def test_reject_crash_window_is_surfaced_and_recovers_idempotently(
    ...
    person_ids = {
        p["id"]
        for p in clean_graph.execute_read(
            "MATCH (n:Person {tenant_id: $t}) RETURN n.id AS id", t=tenant_id
        )
    }
    ...
    assert _counts(sessions, tenant_id) == (2, 0, 0), "Postgres rolled back: still parked"
    ...
    with sessions() as session:
        ...
        judgement = session.execute(
            select(ResolverJudgement.judgement).where(ResolverJudgement.tenant_id == tenant_id)
        ).scalar_one()
    ...
    with sessions() as session:
        again = signoff.reject(
            session, clean_graph, tenant_id=tenant_id, canonical_id=canonical_id, approver="alice"
        )
    assert again.already_applied is True
    assert _counts(sessions, tenant_id) == (0, 1, 1)
    engine.dispose()
...
def test_cross_op_after_crash_is_refused_no_orphan(
    ...
    t1 = "b1-xop-approve-then-reject"
    canonical_1 = _seed_parked(sessions, clean_graph, t1)
    with sessions() as session:
        _crash_first_commit(session)
        with pytest.raises(RuntimeError, match="WM_CRASH_AFTER"):
            signoff.approve(
                session, clean_graph, tenant_id=t1, canonical_id=canonical_1, approver="alice"
            )
    with sessions() as session, pytest.raises(signoff.SignOffError, match="re-run approve"):
        signoff.reject(session, clean_graph, tenant_id=t1, canonical_id=canonical_1, approver="bob")

    # Crashed REJECT (member nodes written) → approve must refuse, not orphan the members.
    t2 = "b1-xop-reject-then-approve"
    canonical_2 = _seed_parked(sessions, clean_graph, t2)
    with sessions() as session:
        _crash_first_commit(session)
        with pytest.raises(RuntimeError, match="WM_CRASH_AFTER"):
            signoff.reject(
                session, clean_graph, tenant_id=t2, canonical_id=canonical_2, approver="alice"
            )
    with sessions() as session, pytest.raises(signoff.SignOffError, match="re-run reject"):
        signoff.approve(
            session, clean_graph, tenant_id=t2, canonical_id=canonical_2, approver="bob"
        )
