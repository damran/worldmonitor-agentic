"""Integration tests for tenant-scoped graph read queries."""
...
def test_get_entity_neighbors_and_provenance(clean_graph: Neo4jClient, tenant_id: str) -> None:
    ...
    # get_entity returns the resolved node with its name, tenant, and anchor.
    node = get_entity(clean_graph, tenant_id=tenant_id, entity_id="p1")
    ...
    provenance = get_provenance(clean_graph, tenant_id=tenant_id, entity_id="p1")
    ...
    neighbor_ids = {
        n["id"] for n in get_neighbors(clean_graph, tenant_id=tenant_id, entity_id="p1")
    }
    assert "c1" in neighbor_ids

    # Tenant isolation: another tenant sees nothing.
    assert get_entity(clean_graph, tenant_id="other-tenant", entity_id="p1") is None
