...
def test_edge_to_merged_member_is_rewritten_to_canonical(
    ...
    with sessions() as session:
        stats = resolve_pending(session=session, neo4j=clean_graph)
    ...
    owns = clean_graph.execute_read(
        "MATCH (o:Entity {id: 'ivan'})-[r:OWNS]->(m:Entity) "
    ...
    neighbours = {n["id"] for n in get_neighbors(clean_graph, entity_id="ivan")}
...
def test_block_mode_parks_flagged_cluster_and_does_not_materialise_its_edge(
    ...
    persons = clean_graph.execute_read("MATCH (n:Person) RETURN count(n) AS n")
...
def test_rerun_is_idempotent_noop(clean_graph: Neo4jClient, postgres_dsn: str) -> None:
    ...
    with sessions() as session:
        first = resolve_pending(session=session, neo4j=clean_graph)
    ...
    def _counts() -> tuple[int, int]:
        ...
        edges = clean_graph.execute_read("MATCH ()-[r]->() RETURN count(r) AS n")[0]["n"]
        return nodes, edges

    before = _counts()

    with sessions() as session:
        second = resolve_pending(session=session, neo4j=clean_graph)
